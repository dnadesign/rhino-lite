...
class RhinoSubmittedAssessment extends SubmittedForm
{
    ...
    public function getAssessmentMark()
    {
        ...
        if ($unmarked > 0) {
            return sprintf('%s unmarked answers', $unmarked);
        }

        if ($this->getAnswers()->Count() == 0) {
            return sprintf('Answers deleted!');
        }
    ...
    public function getMarkedAnswers()
    {
        ...
        $markedAnswers = $this->getAnswers()->filter(array('Name' => $questions->column('Name')))->exclude('Mark',
            'none');
